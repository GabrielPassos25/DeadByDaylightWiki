...
import Killers from './Killers.js'
const Characters = () =>{
    const [estado,setarEstado] = useState('Characters');
    if(estado == 'Characters'){
        return(
            <View style={{paddingBottom:70, width: Dimensions.get('window').width, height:Dimensions.get('window').height,backgroundColor:'#29292b'}}>
                <View style ={{marginTop:15, marginLeft:0}}> 
                    <View>
                        <TouchableOpacity style={{flexDirection:'row', justifyContent:'center', marginLeft:10}} onPress={() => setarEstado('Killers')}>
    ...
    else if(estado == 'Killers'){
        return(
            <Killers></Killers>
        )
    }
    else if(estado == 'Survivors'){
        return(
            <Survivors></Survivors>
        )
    }
